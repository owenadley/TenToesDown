...
<div class="campusmenu-body">
  ...
  //check requested forum Id
  //get info for specific forum requsted from db
  if (isset($_GET['id'])) {
    $thisforumid = $_GET['id'];

    $foruminfo = "SELECT * FROM `artist-forums` WHERE `aforum_id` = $thisforumid LIMIT 1";
    $getforuminfo = mysqli_query($conn, $foruminfo);
    $forumrow = mysqli_fetch_assoc($getforuminfo);

    if ($forumrow > 0) {
      echo "
      <div class='aforum-titlewrapper'>
        <h1>" . $forumrow['aforum_title'] . "</h1>
      </div>

      <br><hr><br>

      <p>" . $forumrow['aforum_description'] . "</p>
...
";
      ...
      echo "Could not find this forum.";
    }
  }
